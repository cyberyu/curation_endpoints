...
class MajorityVoterRev(BaseAggregator):
    """Simple aggregator based on majority voting except changed so it doesn't look at '0' predictions unless that's the only 
    prediction """
    ...
    def __init__(self, name: str, labels: List[str], sequence_labelling: bool = True,
        ...
        super(MajorityVoterRev, self).__init__(
            name, labels, sequence_labelling, prefixes)
        self.weights = initial_weights if initial_weights else {}
    ...
    def _aggregate(self, obs: pandas.DataFrame, coefficient=0.1) -> pandas.DataFrame:
        ...
        # MAIN CHANGE: want to alter this so that 'O' only has predictions if nothing else does
        label_votes[:,0] = (np.sum((label_votes[:,1:] != 0).astype(int), axis=1) == 0).astype(int)
