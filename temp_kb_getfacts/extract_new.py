...
def analyze_results(text, results):
    '''
...
#             print(tri)
            ...
            prob = entailment_prob([premise, hypothesis, None])
...
def get_facts(input_text, model_type='bert', use_filter=False, tree_weight=0.0, debug=False, filter=False):
    """
...
#     print('input text:', input_text)
    ...
    include_sentence = info.get('include_sentence', False)

    threshold = info.get('threshold', 0.003)
    ...
    language_model = model_mapping[model_type]

    outputs = list()

#     tokenizer = AutoTokenizer.from_pretrained(language_model)
    tokenizer = tokenizer_dict[model_type]
    encoder = model_dict[model_type]
#     if 'gpt2' in language_model:
#         encoder = GPT2Model.from_pretrained(language_model)
...
#         print(f'line: {sentence}')
        if len(sentence):
            ...
            for sent in nlp(sentence).sents:
                ...
                # Match
                for triplets in parse_sentence(sent.text, tokenizer, encoder, nlp, use_cuda=use_cuda, use_filter=use_filter, tree_weight=tree_weight, debug=debug):
                    valid_triplets.append(triplets)
            ...
            if len(valid_triplets) > 0:
                ...
                for triplet in valid_triplets:
                    ...
                    if debug: print(f'conf: {conf}, threshold: {threshold}')
                    if conf < threshold:
                        continue
...
#                   g.write(json.dumps( output )+'\n')
    ...
    if filter:
        refined_output = analyze_results(text=input_text, results=outputs)
...
if __name__ == '__main__':
    ...
    with open('/mnt/github_kg/lm_kg_construction/docred_10/A_Charlie_Brown_Christmas.txt', 'r') as f:

        text = f.readlines()

    info = {
        'text': 'Playoff hockey is the hardest sport to watch. Especially, when the Vancouver Canucks are playing against the Boston Bruins.',
        #    'text': text
    }
    ...
    print(text)
    #text = 'Alteryx Inc. AYX, +3.81% shares rallied in the extended session Monday after the data-analytics software company said Mark Anderson is succeeding Dean Stoecker as chief executive effective immediately and raised its sales guidance. '
    text = 'Alteryx Inc. AYX, +0.69% shares rallied in the extended session Monday after the data-analytics software company said Mark Anderson is succeeding Dean Stoecker as chief executive effective immediately and raised its sales guidance.'

    s = time.time()
#     export_file()
    extracted_triplets = get_facts(text, filter=True)
    
    
    for tpl in extracted_triplets:
        for tt in tpl['tri']:
            ttt=tt['h']+' '+tt['r'].replace('_', ' ')+' '+tt['t']
            single_sentence_encodings = tokenizer(ttt, return_tensors="pt")
            sppl=fixsize_perplexity(single_sentence_encodings)
            
            print(ttt + '  ' + str(sppl))
            
        #print(tpl['tri']['h']+' '+tpl['tri']['r']+' '+tpl['tri']['t'])

    e = time.time()
    print(f'Execution time: {round(e-s, 4)}')
