...
=======
import copy 
>>>>>>> 3a28b5d66deef7cde67107c8b63bf23eca1511ea
import os
import json
import numpy as np
...
def to_docs(texts, labs, ignore_annotators=[], labels=[]):    
    ...
    for doc, labs_ in zip(docs, labs):
        for annotator, span_list in labs_.items():
            if any(x in annotator for x in ignore_annotators): # ignore_annotators contains subsets of what we want to match
                continue
            ...
            for span in span_list:
                ...
                s = doc.char_span(span['pos'][0], span['pos'][1] + 1, label=span['name']) # span['pos'] includes final index so +1 here for slice indices
                if s is not None:
                    doc.spans[annotator].append(s)
                    unique_labs.add(span['name'])
    #print(docs[0].spans)
...
=======
        d['gold_standard'] = data_dict['val_true_labels'][i] # doing this for now only for compatibility with other code
...
#     S3_OBJ = S3Utils(AMAZON_PROFILE, BUCKET, base_folder)
#     return S3_OBJ
=======
def create_s3_obj(base_folder):
    from S3Utils.S3Utils import S3Utils
